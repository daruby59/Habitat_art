...
//
// csPlotFunction2(constant,linear,quadratic,cubed,inverse,invsq,start,end,wt,cl)
//
function csPlotFunction2(constant,linear,quadratic,cubed,inverse,invsq,start,end,wt,cl) { 
	var canvas = document.getElementById('Smartart');
                if (canvas.getContext){
                  var gS2 = canvas.getContext('2d');
                }
	select_color(cl);
                ...
                gS2.beginPath();        
                var  k = start+0.01;
	// (Math.sqrt(2*3.1416)))*Math.pow(2.718,(-0.5)*y*y)
                var y_plot = constant + linear*k + quadratic*Math.pow(k,2) + cubed*Math.pow(k,3) + inverse/k+(invsq/(Math.sqrt(2*3.1416)*Math.pow(2.718,(-0.5*k*k))));
                gS2.moveTo(40+start*(x_pos),cartesian_size+40-y_plot*(y_pos));
                for (k=start;k<=end;k=k+1){
                  y_plot = constant + linear*k + quadratic*Math.pow(k,2) + cubed*Math.pow(k,3) + inverse/k+ (invsq/(Math.sqrt(2*3.1416)*Math.pow(2.718,(-0.5*k*k))));
                  gS2.lineTo(40+k*(x_pos),cartesian_size+40-y_plot*(y_pos));
                }
                gS2.closePath;
                gS2.stroke();
...
// coordinate system
//
function coordinate(x_scale,y_scale,x_step,y_step,x_grid_flag,y_grid_flag,x_axis_flag,y_axis_flag,x_value_flag,y_value_flag,size_temp) {
                var canvas = document.getElementById('Smartart');
                if (canvas.getContext){
                  var gS2 = canvas.getContext('2d');
                }
